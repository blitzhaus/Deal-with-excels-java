...
public class Rep9282015 {
	...
	public Rep9282015() throws IOException{
		...
		while((nextLine = reader.readNext()) != null){
			if ( (nextLine[0].equals(null))||(nextLine[0].equals("")) ||(nextLine[1].equals(null)) || (nextLine[1].equals(""))
				...
				i++;
				if (jobIDList.contains(nextLine[0])){
					
				} else {
					jobIDList.add(nextLine[0]);
				}
		...
		for(int jobListCount = 0; jobListCount < jobIDList.size(); jobListCount++){
			...
			while ((nextLine = reader.readNext()) != null) {
				...
				if(nextLine[0].equals(jobIDList.get(jobListCount))){
					System.out.println("This is it");
				}

				if ( (nextLine[0].equals(jobIDList.get(jobListCount)) == false) || (nextLine[0].equals(null))||(nextLine[0].equals("")) ||(nextLine[1].equals(null)) || (nextLine[1].equals(""))
						||(nextLine[2].equals(null)) || (nextLine[2].equals("")) ||(nextLine[4].equalsIgnoreCase("A") == false)) {
			...
			while ((i < data.length) && ((nextLine = reader.readNext()) != null)) {
				...
				if ( (nextLine[0].equals(jobIDList.get(jobListCount)) == false) || (nextLine[0].equals(null))||(nextLine[0].equals("")) ||(nextLine[2].equals(null)) || (nextLine[2].equals(""))
			...
			writeToSheet(data, total, jobIDList.get(jobListCount));
	...
	private void writeToSheet(String[][] data, int total, String workBookName) throws IOException {
		// TODO Auto-generated method stub
		HSSFWorkbook writableWorkbook = null;
		
		int rows = 0;
		writableWorkbook = createWorkBook(workBookName);
	...
	private void count(String[][] data,
		...
		int rows = 0;
		writableWorkbook = createWorkBook(data[0][0]);
	...
	private HSSFWorkbook createWorkBook(String workBookName) throws FileNotFoundException {
		// TODO Auto-generated method stub

		fileOut = new FileOutputStream(workBookName+"_output.xls");
