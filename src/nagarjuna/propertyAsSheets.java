...
public class propertyAsSheets {

	FileOutputStream fileOut;

	public static void main(String[] args) throws IOException,
			RowsExceededException, WriteException {
		new propertyAsSheets();

	}

	public propertyAsSheets() throws IOException, RowsExceededException, WriteException {
		...
		while ((nextLine = reader.readNext()) != null) {
			...
			if ((nextLine[1].equals(null)) || (nextLine[1].equals("")) || (nextLine[3].equalsIgnoreCase("A") == false)) {
		...
		data = new String[i][4];
		reader.close();
		reader = new CSVReader(new FileReader("ajith.csv"));
...
*/
	...
	public void count(String[][] data) throws RowsExceededException,
		...
		for (int i = 2; i < data.length; i++) {
			...
			if ((i == 2)
					|| ((i < data.length - 1) && (property
							.equalsIgnoreCase(data[i - 1][0]) == false))) {
			...
			while ((i < data.length - 2)
					&& (property.equalsIgnoreCase(data[i + 1][0]))
					&& (entitlement.equalsIgnoreCase(data[i + 1][1]))) {
			...
			if ((i < data.length - 1)
					&& (entitlement.equalsIgnoreCase(data[i + 1][1]) == false)) {
				entitlementNumber++;
			}
			/*
			...
			/*
			 * Number numbOfUsers = new Number(1, entitlementNumber,
			 * numberOfUsersForTHisEntitlement);
